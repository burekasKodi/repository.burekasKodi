...
logger.debug(os.path.join(__cwd__,'ignore.txt'))
...
logger.debug('start')
logger.debug("Loading '%s' version '%s'" % (__scriptname__, __version__))
...
def PrintException():
    ...
    logger.debug( 'EXCEPTION IN ({}, LINE {} "{}"): {}'.format(filename, lineno, line.strip(), exc_obj))
...
def isExcluded(movieFullPath):
    ...
    logger.debug("isExcluded(): Checking exclusion settings for '%s'." % movieFullPath)
    check_local=all_setting["local_files"]=='true'

    if not check_local and 'smb:' in movieFullPath:
        logger.debug("isExcluded(): Video is Local. Do not exclude.")
        # logger.debug("isExcluded(): Video is Local.")
        return False #was True

    if (movieFullPath.find("pvr://") > -1) :
        logger.debug("isExcluded(): Video is playing via Live TV, which is currently set as excluded location.")
        return True

    logger.debug('Excluded Check:')
    logger.debug('current_list_item: ' + repr(current_list_item.lower()))
    logger.debug('excluded_addons: ' + repr(excluded_addons))

    if (xbmc.getInfoLabel("VideoPlayer.mpaa")=='heb'):
        logger.debug("isExcluded(): Excluded from list defined!!." )
        return True

    if any(x in current_list_item.lower() for x in excluded_addons):
        logger.debug("isExcluded(): Video is playing from '%s', which is currently set as !!excluded_addons!!." )
        return True
    ...
    if ExcludeAddos and getSettingAsBool('ExcludeAddosOption'):
        if ((ExcludeAddos.lower()) in current_list_item.lower() ):
            logger.debug("isExcluded(): Video is playing from '%s', which is currently set as excluded path 1." % ExcludeAddos)
            return True
    ...
    if ExcludeAddos2 and getSettingAsBool('ExcludeAddosOption2'):
        if ( (ExcludeAddos2.lower()) in current_list_item.lower() ):
            logger.debug("isExcluded(): Video is playing from '%s', which is currently set as excluded path 2." % ExcludeAddos2)
            return True
    ...
    if ExcludeAddos3 and getSettingAsBool('ExcludeAddosOption3'):
        if ((ExcludeAddos3.lower()) in current_list_item.lower()   ):
            logger.debug("isExcluded(): Video is playing from '%s', which is currently set as excluded path 3." % ExcludeAddos3)
            return True
    ...
    if ExcludeAddos4 and getSettingAsBool('ExcludeAddosOption4'):
        if ((ExcludeAddos4.lower()) in current_list_item.lower()  ):
            logger.debug("isExcluded(): Video is playing from '%s', which is currently set as excluded path 4." % ExcludeAddos4)
            return True
    ...
    if ExcludeAddos5 and getSettingAsBool('ExcludeAddosOption5'):
        if ((ExcludeAddos5.lower()) in current_list_item.lower()  ):
            logger.debug("isExcluded(): Video is playing from '%s', which is currently set as excluded path 5." % ExcludeAddos5)
            return True
    ...
    if getSettingAsBool('ExcludeAddosOption6'):
        ...
        for items in ExcludeAddos6:
            if ((items.lower()) in current_list_item.lower()  ) and len(items)>0:
                logger.debug("isExcluded(): Video is playing from '%s', which is currently set as excluded path 6." % items)
                return True
        ...
        if all_setting["autosub"]=='true':
            try:
                if self.run:
                    ...
                    logger.debug("movieFullPath '%s'" % movieFullPath)
                    xbmc.sleep(1000)
                    availableLangs = xbmc.Player().getAvailableSubtitleStreams()

                    logger.debug("availableLangs '%s'" % availableLangs)
                    totalTime = xbmc.Player().getTotalTime()
                    #totalTime ==0.0
                    if xbmc.Player().isPlaying():
                        logger.debug('Start Playing')
                        #vidtime = xbmc.Player().getTime()
                        #logger.debug('vidtime:'+str(vidtime))
                        while 1:
                            vidtime = xbmc.Player().getTime()
                            logger.debug('vidtime:'+str(vidtime))
                            if vidtime>0:
                                totalTime = xbmc.Player().getTotalTime()
                                #logger.debug('totalTimeIn:'+str(totalTime))
                                break
                            if not xbmc.Player().isPlaying():
                                ...
                                break
                            xbmc.sleep(100)
                    logger.debug("totalTime '%s'" % totalTime)
                    logger.debug("ExcludeTime '%s'" % ExcludeTime)
                    ...
                    try:
                        ...
                        logger.debug('AutoSubs isExcluded ERROR')

                    logger.debug('Is autosub Playing:'+str(xbmc.Player().isPlaying()))
                    logger.debug('totalTime:'+str(totalTime))
                    logger.debug('movieFullPath:'+str(movieFullPath))
                    logger.debug('(isExcluded(movieFullPath)) ): '+repr(is_excluded))
                    logger.debug('force_download:'+str(force_download) )

                    if (xbmc.Player().isPlaying() and totalTime > ExcludeTime and force_download==True and not is_excluded ):
                        self.run = False
                        #xbmc.sleep(1000)
                        logger.debug('Started: AutoSearching for Subs')
                        logger.debug('running')
                        logger.debug(repr(running))
                        running=0
                        if running==0:
                            running=1
                          #try:
                        ...
                        logger.debug('Started: Subs found or Excluded')
                        self.run = False
            except:
                pass
...
def get_aa_server_ch():
    ...
    if x_pre_tv is not None:
        logger.debug('-----------------Done Update AA TV: %s-----------------'%(len(x_pre_tv)))
    else:
        logger.debug('-----------------Failed Update AA TV')

    if x_pre_movie is not None:
        logger.debug('-----------------Done Update AA MOVIE: %s-----------------'%(len(x_pre_movie)))
    else:
        logger.debug('-----------------Failed Update AA MOVIE')

    return 'OK'
'''

xbmc.sleep(100)
logger.debug('Action Autosubs:'+str(action))
if action=='search':
...
if action is None:
    ...
    logger.debug('AutoSubs service_started')
